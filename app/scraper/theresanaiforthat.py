...
class TheresAnAIForThatScraper:
    ...
    def extract_features(self, description: str, tags: List[str]) -> List[str]:
        ...
        feature_keywords = [
            ...
            "natural language", "image processing", "text generation"
        ]
    ...
    async def scrape_tool_page(self, tool_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if description_elem:
                desc_text = description_elem.get('content') if description_elem.get('content') else description_elem.get_text()
            ...
            tags = []
            tag_elements = soup.find_all(['span', 'div'], {'class': re.compile(r'tag|category|label', re.I)})
            ...
            pricing_text = ""
            pricing_elem = soup.find(['div', 'span', 'p'], {'class': re.compile(r'pric|cost|free|paid', re.I)})
            if pricing_elem:
                pricing_text = pricing_elem.get_text()
            
            pricing_info = self.extract_pricing_info(pricing_text + " " + tool_data.get('description', ''))
            ...
            tool_data['quality_score'] = min(10, max(1, 5 + len(tool_data.get('features', [])) // 2))
